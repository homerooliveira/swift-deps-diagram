...
func TestErrorToExitCodeMapping(t *testing.T) {
	...
	if code := ExitCode(New(KindBazelWorkspaceNotFound, "missing bazel workspace", nil)); code != 1 {
		t.Fatalf("expected code 1 for bazel workspace not found, got %d", code)
	}
	if code := ExitCode(New(KindBazelQueryFailed, "query failed", errors.New("x"))); code != 2 {
		t.Fatalf("expected code 2 for bazel query failed, got %d", code)
	}
